...
try:
    get_match_info(
        "/match/3901212/matchfacts/arsenal-vs-leeds-united"
    )  # Testing on a Premier league match
    get_match_info(
        "/match/3918204/matchfacts/elche-vs-barcelona"
    )  # Testing on a LaLiga match
    get_match_info(
        "/match/3919315/matchfacts/napoli-vs-milan"
    )  # Testing on a Serie-A match
...
try:
    get_player_bio(
        "/players/737066/erling-braut-haaland"
    )  # testing on Erlig Braut Haaland
...
try:
    dtg = gather_dtg(
        tokenize_page(
            "https://www.fotmob.com/match/3901212/matchfacts/arsenal-vs-leeds-united",
            match=True,
        )[:50]
    )
...
try:
    ...
    if league["name"] == "Premier League" and league["id"] == "47":
        ...
        print(
            Fore.RED
            + f"    gather_league() expected 'Permier League' and 'id' -> got {league['name']} and {league['id']}"
        )
except Exception as e:
...
try:
    matchStats = gather_match_statistics(
        tokenize_page(
            "https://www.fotmob.com/match/3901212/matchfacts/arsenal-vs-leeds-united",
            match=True,
        )[:6000]
    ...
    if matchStats == {
        ...
        "xG": {
            "expected goals": ["3.78", "0.72"],
            "first half": ["1.40", "0.17"],
            "second half": ["2.38", "0.56"],
            "open play": ["2.49", "0.72"],
            "set play": ["0.50", "0.00"],
            "penalty": ["0.79", "0.00"],
            "on target": ["3.39", "0.48"],
        ...
        "passes": {
            "passes": ["636", "307"],
            "accurate passes": ["544", "86", "%", "220", "72"],
            "own half": ["227", "118"],
            "opposition half": ["317", "102"],
            "accurate long balls": ["21", "54", "%", "19", "40"],
            "accurate crosses": ["6", "55", "%", "1", "10"],
            "throws": ["14", "20"],
        },
        "defence": {
            "tackles won": ["9", "60", "%", "12", "57"],
            "interceptions": ["8", "13"],
            "blocks": ["1", "6"],
            "clearances": ["17", "13"],
            "keeper saves": ["4", "1"],
        ...
        "duels": {
            "duels won": ["60", "50"],
            "ground duels": ["40", "51", "%", "38", "49"],
            "aerial duels": ["20", "63", "%", "12", "38"],
            "successfull dribbles": ["13", "50", "%", "6", "50"],
        },
        "cards": {"yellow cards": ["0", "2"], "red cards": ["0", "0"]},
    }:
...
try:
    playerBio = gather_player_bio(
        tokenize_page("https://www.fotmob.com/players/737066/erling-braut-haaland")
    )
    if playerBio["bio"] == {
        "position": "CF",
        "Height": "194",
...
try:
    playerPreformance = gather_player_performance(
        tokenize_page(
            "https://www.fotmob.com/match/3901212/matchfacts/arsenal-vs-leeds-united",
            match=False,
        )[2000:]
