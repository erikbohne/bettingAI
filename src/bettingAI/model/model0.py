...
def main(session: sqlalchemy.orm.Session) -> None:
    ...
    matches = session.execute(
        sqlalchemy.text(
            "SELECT id, home_team_id, away_team_id, season, date FROM matches"
        )
    ).fetchall()
    print(len(matches))
    ...
    for epoch in range(epochs):
        for batch in range(int(len(matches) / BATCHSIZE)):
            ...
            print(
                f"Batch: {batch}/{len(matches)/BATCHSIZE} complete with loss -> {loss.item()} in a time of -> {elapsed_time:.2f}"
            )
    ...
    def generate(self) -> Tuple[List[Any], List[Any]]:
        while True:
            ...
            for i in range(0, len(self.matches), self.batch_size):
                ...
                Y_batch = []
                batch = 1
                for matchID, teamID, opponentID, season, date in self.matches[
                    i : +self.batch_size
                ]:
                    X_batch.append(
                        features_for_model0(
                            teamID, opponentID, season, "home", date, session
                        )
                    )
                    Y_batch.append(
                        labels(matchID, teamID, session)
                    )  # returns the labels for the match
                    X_batch.append(
                        features_for_model0(
                            opponentID, teamID, season, "away", date, session
                        )
                    )
                    Y_batch.append(
                        labels(matchID, opponentID, session)
                    )  # returns the labels for the match
                    batch += 1
